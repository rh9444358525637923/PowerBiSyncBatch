...
### MSAL でサポートされている認証フロー一覧

| 認証フロー | 概要 | サポートされるアプリ |
| :--- | :--- | :--- |
| **Authorization code（承認コード）** | ユーザーがサインインし、ユーザーの代わりに Web API にアクセスします。 | デスクトップ, モバイル, SPA, Web |
| **Client credentials（クライアント資格情報）★** | **アプリ自体の ID を使用したアクセス。ユーザー操作を伴わない自動バッチ処理（デーモン）に使用します。（本システムで採用）** | **デーモン (Daemon)** |
| **Device code** | 入力に制約のあるデバイス（IoT等）でユーザーに代わってサインインします。 | デスクトップ, モバイル |
| **On-behalf-of (OBO)（代理 (OBO)）** | ユーザーの代理として、API から別の API へアクセスします。 | Web API |
| **Username/password (ROPC)** | アプリケーションがパスワードを直接処理します。<br>※**セキュリティリスクのため非推奨** | デスクトップ, モバイル |
| **統合 Windows 認証** | ドメイン参加PCでユーザーの UI 操作なしで（サイレントに）トークンを取得します。 | デスクトップ, モバイル |

> Note: ID/パスワード方式（ROPC）の非推奨について
> プログラム内に直接「ユーザーID」と「パスワード」を記述して認証を行う方式（リソース所有者パスワード資格情報フロー: ROPC）は、
> セキュリティ上の重大なリスクがあるため、Microsoft の公式ドキュメントで**明確に非推奨**とされています。
>
> **非推奨の主な理由:**
> 1. **多要素認証 (MFA) に非対応:** 現在主流の MFA（スマホ認証など）が要求される環境では、プログラムが画面を突破できずエラーになります。
> 2. **パスワード漏洩リスク:** アプリケーション（ソースコードや設定ファイル）にユーザーの生パスワードを保持することは極めて危険です。
> 3. **運用負荷の増大:** 企業ポリシーによる定期的なパスワード変更のたびに、プログラムの改修や再デプロイが必要になります。
> 
> そのため本システム（デーモンアプリ）では、無人バッチ処理における Microsoft のベストプラクティスである**「サービスプリンシパル（クライアント資格情報フロー）」**を採用し、安全かつ安定した自動実行を実現しています。

* 公式ドキュメント：[MSAL でサポートされている認証フロー](https://learn.microsoft.com/ja-jp/entra/msal/msal-authentication-flows)
* 公式ドキュメント：[SQL 認証の代替え手段とする Microsoft Entra 認証](https://learn.microsoft.com/ja-jp/fabric/data-warehouse/entra-id-authentication)


### Entra ID における認証方式の比較表
